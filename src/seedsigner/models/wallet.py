    ...
    def process_camera_data(self):
        ...
        if data[0] != "nodata":
            ...
            if not self.capture_complete() and self.scan_display_working == 0:
                ...
                View.draw.text(((self.WIDTH - tw) / 2, 15), "Collecting QR Codes:", fill="ORANGE", font=View.IMPACT22)
                lines = textwrap.wrap("".join(self.frame_display), width=11)
                yheight = 60
                for line in lines:
                    tw, th = View.draw.textsize(line, font=View.COURIERNEW30)
                    View.draw.text(((self.WIDTH - tw) / 2, yheight), line, fill="ORANGE", font=View.COURIERNEW30)
                    yheight += 30
                tw, th = View.draw.textsize("Right to Exit", font=View.IMPACT18)
                View.draw.text(((self.WIDTH - tw) / 2, 215), "Right to Exit", fill="ORANGE", font=View.IMPACT18)
                View.DispShowImage()
                self.scan_display_working = 0
